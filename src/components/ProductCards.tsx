...
export function ComboCard({combo}:{combo:Combo}){
  ...
  return <article className="card combo"><div className="combo-top"><strong>{combo.code}</strong>{saving>0&&<small>ECONOMIZE {formatBRL(saving)}</small>}</div><ProductImage label={`Combo ${combo.code}`} wide/><div className="card-body"><h3>{combo.name}</h3><p>{combo.description}</p><ul>{combo.slots.map((s,i)=><li key={i}>{s.label}{s.options.length>1?' — à sua escolha':''}</li>)}</ul><div className="price-row"><div>{saving>0&&<del>{formatBRL(list)}</del>}<strong>{formatBRL(combo.price)}</strong></div><small>DISPONÍVEL</small></div><button className="btn primary" onClick={()=>setOpen(true)}>COMPRAR</button><button className="btn outline" onClick={()=>setOpen(true)}>MONTAR E ADICIONAR</button></div>{open&&<ConfigureDialog combo={combo} onClose={()=>setOpen(false)}/>}</article>
}
